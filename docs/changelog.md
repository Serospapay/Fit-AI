...
- Пакет zod-схем (`goal`, `reminder`, `recommendation`) для перевірки користувацьких даних до попадання в бізнес-логіку
- Інтеграційні тести на Jest + Supertest для цілей користувача та експорту (Excel/PDF)
- In-memory сервіс кешування (`services/cache.ts`) з TTL для популярних запитів
- Підсумкові вкладки та секції з ключовими метриками, діапазоном дат і застосованими фільтрами в експортованих файлах (Excel/PDF)
- Сервісний воркер для нагадувань (`public/reminder-sw.js`) та локальне планування браузерних сповіщень
- Розширена модель нагадувань: таймзона, інтервал повторень, дата початку/завершення, канал сповіщень
...
- Контролери цілей, нагадувань та рекомендацій тепер виконують повну перевірку тіла та query-параметрів через Zod
- Експорт тренувань/харчування приймає додаткові фільтри (`type`, `status`, `minRating`, `mealType`, `minCalories`, `minMacro` тощо) без дублювання коду
- Мотиваційні цитати отримуються з кешу замість повторних звернень до БД
- Інтерфейс нагадувань отримав попередній перегляд повторень, тестове сповіщення, блок «Найближчі спрацювання» та гнучкі налаштування повтору
...
- Заблоковано збереження некоректних значень (дата завершення, дні тижня, статуси) завдяки централізованим схемам
- Перевірено критичні сценарії (створення/перегляд цілей, експорт тренувань і харчування) автоматизованими тестами
